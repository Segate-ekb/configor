...
Процедура ЗарегистрироватьКлассПараметров(Знач КлассОбъект, Знач Префикс) Экспорт
	...
	ПроверитьСвободенЛиПрефикс(КлассОбъект, Префикс);
...
// Использует зарегистрированные описания полей и префикс для маппинга
//
// Параметры:
//   КлассОбъект - Объект - экземпляр DTO-класса для заполнения,
//                          ранее переданный в ЗарегистрироватьКлассПараметров
//
Процедура ЗаполнитьОбъектПараметров(Знач КлассОбъект) Экспорт

	Запись = РеестрКлассовПараметров[КлассОбъект];

	Если Запись = Неопределено Тогда
		ВызватьИсключение "Класс параметров не зарегистрирован,"
			+ " перед заполнением вызовите ЗарегистрироватьКлассПараметров";
	КонецЕсли;

	ЗаполнитьПоОписаниям(КлассОбъект, Запись.ОписанияПолей, Запись.Префикс);
...
// Проверяет, что префикс не занят другим зарегистрированным классом параметров
// Повторная регистрация того же объекта разрешена и заменяет прежнюю запись
// Префиксы сравниваются без учета регистра, так как индекс параметров регистронезависим
//
// Параметры:
//   КлассОбъект - Объект - регистрируемый экземпляр DTO-класса
//   Префикс - Строка - ключ верхнего уровня в конфигурации
//
Процедура ПроверитьСвободенЛиПрефикс(Знач КлассОбъект, Знач Префикс)

	Для Каждого ЭлементРеестра Из РеестрКлассовПараметров Цикл

		Если ЭлементРеестра.Ключ = КлассОбъект Тогда
			Продолжить;
		КонецЕсли;

		Если НРег(ЭлементРеестра.Значение.Префикс) = НРег(Префикс) Тогда
			ВызватьИсключение "Префикс """ + Префикс
				+ """ уже занят другим зарегистрированным классом параметров";
		КонецЕсли;
...
// Заполняет свойства объекта по описаниям полей,
// рекурсивно спускаясь во вложенные DTO
//
// Параметры:
//   Объект - Объект - заполняемый объект
//   ОписанияПолей - Массив из ОписаниеПоляДТО - описания полей этого объекта
//   Префикс - Строка - ключ конфигурации, соответствующий текущему уровню вложенности
//
Процедура ЗаполнитьПоОписаниям(Знач Объект, Знач ОписанияПолей, Знач Префикс)

	ОбъектРефлектор = Новый Рефлектор;

	Для Каждого ОписаниеПоля Из ОписанияПолей Цикл

		ПолныйКлюч = Префикс + "." + ОписаниеПоля.КлючКонфигурации();

		Если ОписаниеПоля.ЭтоВложенныйДТО() Тогда

			ВложенныйОбъект = ОбъектРефлектор.ПолучитьСвойство(Объект, ОписаниеПоля.Имя);

			Если ВложенныйОбъект = Неопределено Тогда
				ВызватьИсключение "Свойство """ + ОписаниеПоля.Имя
					+ """ описано как вложенный DTO, но не инициализировано на момент заполнения";
			КонецЕсли;

			Лог.Отладка("Рекурсивное заполнение вложенного DTO <%1> по ключу <%2>", ОписаниеПоля.Имя, ПолныйКлюч);
			ЗаполнитьПоОписаниям(ВложенныйОбъект, ОписаниеПоля.ОписанияВложенныхПолей, ПолныйКлюч);
			Продолжить;
		...
		Если Не ЗначениеИзКонфига = Неопределено Тогда
			...
			ОбъектРефлектор.УстановитьСвойство(Объект, ОписаниеПоля.Имя, ЗначениеИзКонфига);
		...
		ИначеЕсли Не ОписаниеПоля.Обязательное Тогда
			...
			ОбъектРефлектор.УстановитьСвойство(Объект, ОписаниеПоля.Имя, ОписаниеПоля.ЗначениеПоУмолчанию);

		Иначе

			ВызватьИсключение "Обязательный параметр """ + ПолныйКлюч
				+ """ (свойство """ + ОписаниеПоля.Имя
				+ """) не задан в конфигурации и не имеет значения по умолчанию";
...
// Разбирает строковое представление булева значения
// Строка, не входящая в список допустимых значений, вызывает исключение
//
// Параметры:
//   Значение - Строка - строковое представление булева значения
//
// Возвращаемое значение:
//   Булево - разобранное значение
//
Функция ПривестиСтрокуКБулево(Знач Значение)

	СтрЗначение = ВРег(СокрЛП(Значение));

	Если Не СтрРазделить("1,TRUE,ИСТИНА,ДА,YES,ON", ",").Найти(СтрЗначение) = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;

	Если Не СтрРазделить("0,FALSE,ЛОЖЬ,НЕТ,NO,OFF", ",").Найти(СтрЗначение) = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	ВызватьИсключение "Невалидное булево значение";
	...
	Попытка
		...
		Иначе
			...
			Если ЦелевойТип = Тип("Булево") И ТипЗнч(Значение) = Тип("Строка") Тогда
				Возврат ПривестиСтрокуКБулево(Значение);
			КонецЕсли;
