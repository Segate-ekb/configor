...
Перем РеестрКлассовПараметров; // Соответствие: Префикс -> ОписанияПолей (Массив ОписаниеПоляДТО)
...
Процедура ЗарегистрироватьКлассПараметров(Знач КлассОбъект, Знач Префикс) Экспорт
	...
	СуществующиеОписания = РеестрКлассовПараметров[Префикс];

	Если СуществующиеОписания <> Неопределено Тогда
		Для Каждого Описание Из ОписанияПолей Цикл
			СуществующиеОписания.Добавить(Описание);
		КонецЦикла;
	Иначе
...
Процедура ЗаполнитьОбъектПараметров(Знач КлассОбъект, Знач Префикс) Экспорт
	...
	ОписанияПолей = РеестрКлассовПараметров[Префикс];

	Если ОписанияПолей = Неопределено Тогда
